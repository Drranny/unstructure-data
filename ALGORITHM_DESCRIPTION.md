...
**알고리즘**: **통합 오류 검사 (패턴 + 맞춤법 + 형식 + 맥락)**

```python
1. 기본 패턴 검사:
   - 연속된 공백 (인라인만, 줄바꿈 제외)
   - 한글-영문 불규칙 혼용 (약어 제외: AI, API 등)
   - 누락된 값 (예: "__명")
   - 과도한 반복 ("ㅋㅋㅋ", "굿굿")
   - 부적절한 표현 ("이딴", "쓰레기" 등)
   - 문법 오류 패턴 (모순, 반복, 불명확한 표현)

2. 맞춤법 검사:
   - 영어: pyenchant 라이브러리 (사전 기반)
     * 없으면 패턴 기반 일반 오탈자 검사
   - 한국어: py-hanspell 라이브러리 (Naver API)
     * 없으면 패턴 기반 일반 오탈자 검사
     * 한글 비율 30% 이상일 때만 검사
...
4. 데이터 내 중복 검사:
   - 문장 단위가 아닌 데이터 값 내 중복
   - 숫자 리스트 중복 (예: "ID: 123, 456, 123")
   - 단어 반복 (예: "그거 그거 그거")

5. 맥락 단절 검사:
   - Sentence Transformer로 연속 문장 간 유사도 계산
   - 유사도 < 0.3이면 맥락 단절로 판단

6. 오류 비율 계산 (개선된 방식):
   오류 비율 = (오류 개수) / (총 단어 수)
   패널티 = 오류 비율 * 0.8 (최대 90%)
   - 오류 3개 이하 & 비율 5% 미만: 패널티 50% 감소
   정확성 = 1.0 - 패널티 (최소 0.05)
```

**특징**:
- ✅ **맞춤법 검사 통합**: 영어/한국어 맞춤법 자동 검사
- ✅ **맥락 분석**: Sentence Transformer 기반 문장 간 유사도 분석
- ✅ **형식 검증**: 날짜 등 형식 일관성 검사
- ✅ **합리적 점수**: 오류 비율 기반 계산으로 과도한 패널티 방지
- ⚠️ **API 의존성**: py-hanspell은 인터넷 연결 필요 (Naver API)
...
3. 불완전한 문장 패턴 검사:
   ...
   - "나는", "그가" 등 (주어만 있는 경우)
4. 의미 있는 문장 비율 계산:
   완전성 = (완전한 문장 수) / (전체 문장 수)
```

**특징**:
- ✅ **패턴 기반**: 기본적인 불완전 문장 감지
- ✅ **간단 명확**: 구현이 쉽고 빠름
- ⚠️ **제한**: 의미 분석은 제한적 (길이와 기본 패턴만)
...
- 각 이미지의 개별 점수 저장 (해상도, 선명도, 노이즈, 중복도, 종합점수)
...
- 정확성: 패턴 검사 + 맞춤법 검사 + 형식 검사 + 데이터 중복 + 맥락 단절
- 중복도: Sentence Transformer로 문장 임베딩 생성 후 계산
- 완전성: 문장 길이 + 불완전 패턴 검사
- 각 텍스트의 개별 점수 저장 (정확성, 중복도, 완전성, 종합점수)
...
| **텍스트 분석** | ✅ 패턴 + 맞춤법 + 형식 + 맥락 검사 | 도메인 특화 검사 |
| **맞춤법 검사** | ✅ 영어/한국어 맞춤법 통합 | - |
| **맥락 분석** | ✅ Sentence Transformer 기반 | - |
| **중복 처리** | 모든 지표 계산 후 중복 감지 | 중복 감지 후 선택적 계산 |
| **개별 점수** | ✅ 각 이미지/텍스트 개별 점수 저장 | - |
| **범용성** | ✅ 높음 | ⚠️ 타입별로 달라짐 |
| **정확도** | ✅ 향상됨 (맞춤법, 맥락 검사 추가) | ✅ 타입별로 향상 가능 |
| **처리 속도** | ✅ 빠름 | ⚠️ 타입 감지로 느려짐 |

**결론**: 
- 현재는 모든 이미지/텍스트에 **동일한 표준 알고리즘**을 사용하고 있으며, 모든 지표를 항상 계산합니다.
- 텍스트 품질 검사는 **맞춤법 검사(pyenchant, py-hanspell)**, **맥락 단절 검사(Sentence Transformer)**, **형식 일관성 검사** 등이 통합되어 정확도가 향상되었습니다.
- 각 이미지/텍스트의 **개별 점수**를 저장하여 리포트에서 전체 평균과 개별 점수를 모두 확인할 수 있습니다.
- 해상도만 다른 동일 이미지도 각각 계산하지만, 중복도로 감지는 가능합니다.
- 필요시 중복 감지 후 효율적인 계산 방식으로 개선할 수 있습니다.
